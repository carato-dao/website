<?php
	...
	if(isset($_SESSION[$session_code]['Username'])){	
		$user = returnDBObject("SELECT * FROM admins WHERE id=?",array($_SESSION[$session_code]['id']));
	}
	if(isset($_COOKIE[$session_code.'_Username'])){	
		$user = returnDBObject("SELECT * FROM admins WHERE id=?",array($_COOKIE[$session_code.'_id']));
	}
